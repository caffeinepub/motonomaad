        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <details
                key={index}
                className="faq-item group border-2 border-border rounded-lg overflow-hidden hover:border-primary/50 transition-all duration-300 motion-safe:animate-fade-in-up motion-safe:hover:shadow-md"
                style={{ animationDelay }}
              >
                <summary className="cursor-pointer list-none px-6 py-5 font-semibold text-lg flex items-center justify-between hover:bg-muted/50 transition-all duration-200">
                  <span>{faq.question}</span>
                  <svg
                    className="w-5 h-5 text-muted-foreground transition-transform duration-300 group-open:rotate-180 motion-reduce:transition-none"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                ...
                <div className="px-6 py-5 pt-0 text-muted-foreground border-t border-border bg-muted/20 motion-safe:animate-fade-in">
                  <p>{faq.answer}</p>
                </div>
              </details>
            );
